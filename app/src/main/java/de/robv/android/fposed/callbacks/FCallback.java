 ...
 */
public abstract class FCallback implements Comparable<FCallback> {
	/**
	...
	public static abstract class Param {
		...
		/** @hide */
		protected Param(CopyOnWriteSortedSet<? extends FCallback> callbacks) {
	...
	public static void callAll(Param param) {
		...
		for (int i = 0; i < param.callbacks.length; i++) {
			try {
				((FCallback) param.callbacks[i]).call(param);
	...
	/** @hide */
	@Override
	public int compareTo(FCallback other) {
		if (this == other)
			return 0;
