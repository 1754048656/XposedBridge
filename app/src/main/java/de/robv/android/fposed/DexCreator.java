...
import static de.robv.android.fposed.FposedHelpers.inputStreamToByteArray;
...
/*package*/ class DexCreator {
	...
	public static File getDefaultFile(String childClz) {
		return new File(DALVIK_CACHE, "fposed_" + childClz.substring(childClz.lastIndexOf('.') + 1) + ".dex");
	}
	...
	public static File ensure(String clz, Class<?> realSuperClz, Class<?> topClz) throws IOException {
		...
		try {
			return ensure("fposed.dummy." + clz + "SuperClass", realSuperClz);
		} catch (IOException e) {
