...
import static de.robv.android.fposed.FposedBridge.hookAllConstructors;
...
import static de.robv.android.fposed.FposedHelpers.closeSilently;
import static de.robv.android.fposed.FposedHelpers.fileContains;
...
import static de.robv.android.fposed.FposedHelpers.getOverriddenMethods;
import static de.robv.android.fposed.FposedHelpers.getParameterIndexByType;
import static de.robv.android.fposed.FposedHelpers.setObjectField;
import static de.robv.android.fposed.FposedHelpers.setStaticBooleanField;
import static de.robv.android.fposed.FposedHelpers.setStaticLongField;
import static de.robv.android.fposed.FposedHelpers.setStaticObjectField;

/*package*/ final class FposedInit {
	...
	/*package*/ static void initForZygote() throws Throwable {
		if (needsToCloseFilesForFork()) {
			FC_MethodHook callback = new FC_MethodHook() {
				@Override
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
					FposedBridge.closeFilesBeforeForkNative();
				}

				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					FposedBridge.reopenFilesAfterForkNative();
				}
			};
		...
		findAndHookMethod(ActivityThread.class, "handleBindApplication", "android.app.ActivityThread.AppBindData", new FC_MethodHook() {
			@Override
			protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
				...
				if (instrumentationName != null) {
					...
					FposedBridge.disableHooks = true;
					return;
				}
		...
		if (Build.VERSION.SDK_INT < 21) {
			findAndHookMethod("com.android.server.ServerThread", null,
					Build.VERSION.SDK_INT < 19 ? "run" : "initAndLoop", new FC_MethodHook() {
						@Override
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							...
							lpparam.classLoader = FposedBridge.BOOTCLASSLOADER;
			...
			findAndHookMethod(ActivityThread.class, "systemMain", new FC_MethodHook() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
					final ClassLoader cl = Thread.currentThread().getContextClassLoader();
					findAndHookMethod("com.android.server.SystemServer", cl, "startBootstrapServices", new FC_MethodHook() {
						@Override
						protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
							SELinuxHelper.initForProcess("android");
							loadedPackagesInProcess.add("android");
							...
							// Huawei
							try {
								findAndHookMethod("com.android.server.pm.HwPackageManagerService", cl, "isOdexMode", FC_MethodReplacement.returnConstant(false));
							} catch (FposedHelpers.ClassNotFoundError | NoSuchMethodError ignored) {}

							try {
								String className = "com.android.server.pm." + (Build.VERSION.SDK_INT >= 23 ? "PackageDexOptimizer" : "PackageManagerService");
								findAndHookMethod(className, cl, "dexEntryExists", String.class, FC_MethodReplacement.returnConstant(true));
							} catch (FposedHelpers.ClassNotFoundError | NoSuchMethodError ignored) {}
						}
					});
				}
		...
		hookAllConstructors(LoadedApk.class, new FC_MethodHook() {
			@Override
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
		...
		if (findFieldIfExists(ZygoteInit.class, "BOOT_START_TIME") != null) {
			setStaticLongField(ZygoteInit.class, "BOOT_START_TIME", FposedBridge.BOOT_START_TIME);
		}
	...
	/*package*/ static void hookResources() throws Throwable {
		...
		if (!FposedBridge.initXResourcesNative()) {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			hookAllMethods(classGTLR, "getOrCreateResources", new FC_MethodHook() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
			...
			hookAllConstructors(classResKey, new FC_MethodHook() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
			...
			hookAllMethods(classGTLR, "getTopLevelResources", new FC_MethodHook() {
				@Override
				protected void beforeHookedMethod(MethodHookParam param) throws Throwable {
			...
			if (Build.VERSION.SDK_INT >= 19) {
				...
				hookAllMethods(classGTLR, "getTopLevelThemedResources", new FC_MethodHook() {
					@Override
					protected void afterHookedMethod(MethodHookParam param) throws Throwable {
		...
		if (Build.VERSION.SDK_INT >= 24) {
			...
			FposedBridge.invalidateCallersNative(methods.toArray(new Member[methods.size()]));
		}

		// Replace TypedArrays with XTypedArrays
		hookAllConstructors(TypedArray.class, new FC_MethodHook() {
			@Override
			protected void afterHookedMethod(MethodHookParam param) throws Throwable {
				TypedArray typedArray = (TypedArray) param.thisObject;
				Resources res = typedArray.getResources();
				if (res instanceof XResources) {
					FposedBridge.setObjectClass(typedArray, XResources.XTypedArray.class);
				}
			}
		});

		// Replace system resources
		XResources systemRes = (XResources) FposedBridge.cloneToSubclass(Resources.getSystem(), XResources.class);
	...
	private static XResources cloneToXResources(FC_MethodHook.MethodHookParam param, String resDir) {
		...
		XResources newRes = (XResources) FposedBridge.cloneToSubclass(result, XResources.class);
	...
	private static void hookXposedInstaller(ClassLoader classLoader) {
		try {
			findAndHookMethod(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "getActiveXposedVersion",
					FC_MethodReplacement.returnConstant(FposedBridge.getXposedVersion()));

			findAndHookMethod(INSTALLER_PACKAGE_NAME + ".XposedApp", classLoader, "onCreate", new FC_MethodHook() {
				@Override
				protected void afterHookedMethod(MethodHookParam param) throws Throwable {
	...
	/*package*/ static void loadModules() throws IOException {
		...
		ClassLoader topClassLoader = FposedBridge.BOOTCLASSLOADER;
		ClassLoader parent;
	...
	private static void loadModule(String apk, ClassLoader topClassLoader) {
		...
		if (dexFile.loadClass(FposedBridge.class.getName(), topClassLoader) != null) {
		...
		ClassLoader mcl = new PathClassLoader(apk, FposedBridge.BOOTCLASSLOADER);
			...
			while ((moduleClassName = moduleClassesReader.readLine()) != null) {
				...
				try {
					...
					if (!IFposedMod.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class doesn't implement any sub-interface of IFposedMod, skipping it");
						continue;
					} else if (disableResources && IFposedHookInitPackageResources.class.isAssignableFrom(moduleClass)) {
						Log.e(TAG, "    This class requires resource-related hooks (which are disabled), skipping it.");
						continue;
					}

					final Object moduleInstance = moduleClass.newInstance();
					if (FposedBridge.isZygote) {
						...
						if (moduleInstance instanceof IFposedHookCmdInit) {
							IFposedHookCmdInit.StartupParam param = new IFposedHookCmdInit.StartupParam();
							param.modulePath = apk;
							param.startClassName = startClassName;
							((IFposedHookCmdInit) moduleInstance).initCmdApp(param);
						}
					}
				} catch (Throwable t) {
